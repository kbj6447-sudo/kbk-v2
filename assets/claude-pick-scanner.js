...
(function() {
  ...
  function calcClaudeScore(item) {
    ...
    const total = rvolScore + rvolVsPricePremium + priceScore + stageScore +
                  vwapScore + trendScore + rsiScore + preSurgeBonus + pullbackBonus + maScore;

    return {
      total: Math.max(0, Math.min(100, Math.round(total))),
      breakdown: { rvolScore, rvolVsPricePremium, priceScore, stageScore, vwapScore, trendScore, rsiScore, preSurgeBonus, pullbackBonus, maScore }
    };
  }
