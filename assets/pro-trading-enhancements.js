...
function normalizeRecommendationCategory(label) {
  ...
  if (!text) return null;
  if (text.includes("?µí•© ìµœì¢… ?„ë³´") || text.toLowerCase().includes("top picks")) return "?µí•© ìµœì¢… ?„ë³´";
  if (text.includes("?¤ì‹œê°??¨í?")) return "?¤ì‹œê°??¨í?";
  if (text.includes("1?¬ëŸ¬ ë¯¸ë§Œ")) return "1?¬ëŸ¬ ë¯¸ë§Œ ??“± ?„ë³´";
  if (text.includes("1?¬ëŸ¬ ?´ìƒ")) return "1?¬ëŸ¬ ?´ìƒ ??“± ?„ë³´";
  if (text.includes("ë§¤ì§‘")) return "ë§¤ì§‘ ?¤ìº??;
  return null;
}

function currentRecommendationCategory() {
  if (isTopPicksViewActive()) return "?µí•© ìµœì¢… ?„ë³´";
  const pathname = window.location.pathname || "";
  if (pathname === "/" || pathname.includes("/scanner/scalp")) return "?¤ì‹œê°??¨í?";
  if (pathname.includes("/scanner/surge-watch-under-1") || pathname.includes("/surge-watch-under-1")) return "1?¬ëŸ¬ ë¯¸ë§Œ ??“± ?„ë³´";
  if (pathname.includes("/scanner/surge-watch-over-1") || pathname.includes("/surge-watch-over-1")) return "1?¬ëŸ¬ ?´ìƒ ??“± ?„ë³´";
  if (pathname.includes("/scanner/accumulation") || pathname.includes("/accumulation")) return "ë§¤ì§‘ ?¤ìº??;
  ...
  if (raw) return raw;
  if (document.querySelector(".candidate-table")) return "?¤ì‹œê°??¨í?";
  return null;
}
...
function topPickReasoning(item, metrics) {
  ...
  if (setup.volumeStarting) reasons.push("ê±°ë˜?‰ì´ ì¦ê??˜ê¸° ?œì‘?ˆìŠµ?ˆë‹¤");
  if (metrics.rvol !== null && metrics.rvol >= 3) reasons.push(`RVOL ${metrics.rvol.toFixed(1)}ë°°ë¡œ ê¸°ì? ?´ìƒ?…ë‹ˆ??);
  if (volumeAccel !== null && volumeAccel >= 65) reasons.push("ê±°ë˜??ê°€?ë„ê°€ ?‘í˜¸?©ë‹ˆ??);
  if (setup.vwapRecovering) reasons.push(setup.vwapAbove ? "VWAP ?„ì—???Œë³µ ?ë¦„?…ë‹ˆ?? : "VWAP ?¬ëŒ?Œë? ?œë„ ì¤‘ì…?ˆë‹¤");
  if (higherLow !== null && higherLow >= 65) reasons.push("Higher Low êµ¬ì¡°ê°€ ê°ì??©ë‹ˆ??);
  if (resurge !== null && resurge >= 65) reasons.push("?Œë¦¼ ???¬ìƒ??ê°€?¥ì„±???ˆìŠµ?ˆë‹¤");
  if (setup.lowRecovery) reasons.push("ê°€ê²©ì´ ?˜ë‹¨ë¶€?ì„œ ?Œë³µ ì¤‘ì…?ˆë‹¤");
  if (compression !== null && compression >= 70) reasons.push("ë°•ìŠ¤ê¶??•ì¶•???•ì¸?©ë‹ˆ??);

  if (setup.highFailed) cautions.push("?´ë? ?¬ê²Œ ?¤ë¥¸ ??ê³ ì ?ì„œ ë°€?¸ìŠµ?ˆë‹¤");
  if (setup.rsi !== null && setup.rsi >= 80) cautions.push("RSI ê³¼ì—´ êµ¬ê°„?…ë‹ˆ??);
  if (setup.extremeRvolWeak) cautions.push("RVOL?€ ?’ì?ë§?ê°€ê²??ë¦„???½í•©?ˆë‹¤");
  if (setup.vwapBelow) cautions.push("VWAP ?„ë˜??ë°”ë¡œ ì§„ì…?€ ?„í—˜?©ë‹ˆ??);
  if (metrics.risk >= 70) cautions.push("ì¶”ê²© ?„í—˜ ?ìˆ˜ê°€ ?’ìŠµ?ˆë‹¤");
  if (metrics.change >= 80) cautions.push("?¨ê¸° ?ìŠ¹ë¥ ì´ ê³¼í•©?ˆë‹¤");
  if (metrics.rvol !== null && metrics.rvol < 3) cautions.push("RVOL ê¸°ì????„ì§ ë¶€ì¡±í•©?ˆë‹¤");
  ...
  return {
    reasons: reasons.length ? reasons.slice(0, 4) : ["ì´ˆì… ?Œë³µ ì¡°ê±´??ê´€ì°?ì¤‘ì…?ˆë‹¤"],
    cautions: cautions.length ? cautions.slice(0, 3) : ["?¤ì œ ì§„ì… ??VWAP/ì²´ê²° ë°˜ì‘ ?¬í™•??],
    decision,
    priority: decision === "ë§¤ìˆ˜ ê°€?? ? 3 : decision === "ê´€ì°? ? 2 : 0,
  };
}
...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .price-row{gap:8px!important;margin-top:10px!important;font-size:.86rem!important;color:#1e293b!important}
.price-row strong{color:#111827!important;font-weight:800!important}
.price-row span{color:#334155!important}
...
function refreshCurrentView() {
  const visibleRefresh = Array.from(document.querySelectorAll("button"))
    .find((button) => /?ˆë¡œê³ ì¹¨|ê°ì‹œ ê°±ì‹ |?„ì²´ ë¶„ì„/.test(button.textContent || "") && button.offsetParent);
  if (visibleRefresh) {
...
function ensureRouteLinks() {
  ...
  const configs = [
    { label: "ë°±í…Œ?¤íŠ¸", path: "/backtest", target: "backtest-panel" },
    { label: "AI ë¶„ì„", path: "/ai-analysis", target: "debug-panel" },
    { label: "?µí•© ìµœì¢… ?„ë³´", path: "/top-picks", target: "kbk-pro-top-picks" },
  ];
...
async function renderTopPicksOnly() {
  ...
  if (!panel) return;
  panel.innerHTML = `<section class="kbk-route-note">?µí•© ìµœì¢… ?„ë³´ë¥?ê³„ì‚°?˜ëŠ” ì¤‘ì…?ˆë‹¤.</section>`;
  try {
    ...
    persistRecommendationSnapshots(items.map(({ item, price, change, volume, finalScore, reasoning }) =>
      recommendationEntryFromItem(item, "?µí•© ìµœì¢… ?„ë³´", {
        price,
        score: finalScore,
    ...
    panel.innerHTML = `
      <section class="accumulation-hero">
        <div>
          ...
          <h2>?µí•© ìµœì¢… ?„ë³´</h2>
          <p class="section-copy">?¤ì‹œê°??¨í? ?„ë³´, ??“± ê°ì‹œ, ë§¤ì§‘/?¨í„´ ?ìˆ˜ë¥??¨ê»˜ ë³´ê³  ê³¼ì—´ ?„í—˜??ê°ì ???¤ì „???°ì„ ?œìœ„?…ë‹ˆ??</p>
        </div>
        <div class="hero-scoreboard">
          <div class="score-card"><span>?„ë³´</span><strong>${items.length}</strong></div>
          <div class="score-card"><span>ê¸°ì?</span><strong>ì´ˆì… ?°ì„ </strong></div>
        </div>
      </section>
      ${items.length ? items.map(({ item, price, change, volume, surge, risk, pattern, finalScore, signalBonus, reasoning }) => `
        <article class="kbk-pro-top-card">
          ...
          <div class="price-row">
            ...
            <span>ê±°ë˜??${compact(volume)}</span>
          </div>
          <div class="kbk-pro-top-grid">
            <div><span>??“± ê°ì‹œ</span><b>${surge}??/b></div>
            <div><span>?¨í„´</span><b>${pattern}??/b></div>
            <div><span>?„í—˜</span><b>${risk}??/b></div>
            <div><span>?µí•©</span><b>${finalScore}??{signalBonus ? ` (${signalBonus > 0 ? "+" : ""}${signalBonus})` : ""}</b></div>
          </div>
          <div class="kbk-pro-top-meta">
            <div><span>? ì • ?´ìœ </span><p>${reasoning.reasons.map(textEscape).join(" Â· ")}</p></div>
            <div><span>ì£¼ì˜ ?”ì¸</span><p>${reasoning.cautions.map(textEscape).join(" Â· ")}</p></div>
            <div class="kbk-pro-top-decision"><span>ìµœì¢… ?ë‹¨</span><b>${textEscape(reasoning.decision)}</b></div>
          </div>
        </article>
      `).join("") : `<section class="kbk-empty-note">?„ì¬ ?µí•© ê¸°ì????µê³¼???„ë³´ê°€ ?†ìŠµ?ˆë‹¤. ?ˆë¡œê³ ì¹¨?¼ë¡œ ?¤ì‹œ ?•ì¸??ì£¼ì„¸??</section>`}
    `;
  } catch (error) {
    panel.innerHTML = `<section class="kbk-empty-note">?µí•© ?„ë³´ ê³„ì‚°???¤íŒ¨?ˆìŠµ?ˆë‹¤: ${textEscape(error.message)}</section>`;
  }
}
...
function handleRoute() {
  ...
  if (path === "/backtest") {
    routeScroll("backtest-panel", "ë°±í…Œ?¤íŠ¸ ?”ë©´?…ë‹ˆ?? ? íƒ ì¢…ëª© ?ëŠ” ?„ì²´ ë¶„ì„ ë²„íŠ¼?¼ë¡œ ?œê·¸???´í›„ ?€ì§ì„???•ì¸?©ë‹ˆ??");
    return;
  }
  if (path === "/ai-analysis") {
    routeScroll("debug-panel", "AI ë¶„ì„ ?”ë©´?…ë‹ˆ?? ?„ë³´ ? ì • ê·¼ê±°?€ ê³„ì‚° ?°ì´?°ë? ?•ì¸?©ë‹ˆ??");
    return;
  }
...
function clarifyEmptyAccumulation() {
  ...
  note.textContent = "?„ì¬ ì¡°ê±´???µê³¼??ë§¤ì§‘/ê¸‰ë“± ì§ì „/?¡ë³´ ?ŒíŒŒ ?„ë³´ê°€ ?†ìŠµ?ˆë‹¤. ?°ì´??ë¡œë”© ?¤ë¥˜ê°€ ?„ë‹ˆ?? ì§€ê¸??œì ???„í„°ë¥?ë§Œì¡±??ì¢…ëª©???†ë‹¤???»ì…?ˆë‹¤.";
...
function syncMonitorPriceBadge(symbol, price) {
  ...
  const label = `?¤ì‹œê°?ë¶„ë´‰ ${money(value)} / ${krwMoneyFromUsd(value)} Â· ${new Date().toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}`;
  const candidates = Array.from(summary.querySelectorAll("*")).filter((node) =>
    node.children.length === 0 && (/?¤ì‹œê°?ë¶„ë´‰/.test(node.textContent || "") || /^\$[\d.]+\s*\/\s*[\d,]+??.test((node.textContent || "").trim()))
  );
...
function syncRvolCopy(panel, quote) {
  ...
  const replacement = rvol === null
    ? `RVOL?€ ?‰ê·  ê±°ë˜???°ì´?°ê? ?†ì–´ ê³„ì‚° ë¶ˆê??…ë‹ˆ?? ?„ì¬ ?„ì  ê±°ë˜?‰ì? ${compact(volume)}?…ë‹ˆ??`
    : `RVOL?€ ${rvol.toFixed(1)}ë°°ì…?ˆë‹¤.`;
  const walker = document.createTreeWalker(panel, NodeFilter.SHOW_TEXT);
  const nodes = [];
  while (walker.nextNode()) nodes.push(walker.currentNode);
  for (const node of nodes) {
    const text = node.nodeValue || "";
    if (/RVOL[?´ì?]?\s*1\.0ë°?.test(text)) {
      node.nodeValue = text.replace(/RVOL[?´ì?]?\s*1\.0ë°?^.??*(\.|?…ë‹ˆ??.)?/g, replacement);
    }
  }
}
...
function fastSignalDecision({ quote, bars, price, vwap, entryLow, entryHigh, recentHigh }) {
  ...
  if (!aboveVwap) {
    return {
      action: "ê¸°ë‹¤ë¦¬ì„¸??,
      tone: "hold",
      headline: "VWAP ?Œë³µ ?„ì…?ˆë‹¤.",
      reason: "ê±°ë˜?‰ì´ ?ˆì–´??VWAP ?„ë˜?ì„œ???¨í? ì§„ì… ? ë¢°?„ê? ?¨ì–´ì§‘ë‹ˆ??",
    };
  }
  if (tooExtended) {
    return {
      action: "ì¶”ê²© ê¸ˆì?",
      tone: "profit",
      headline: "?´ë? ê¸°ì?? ì—??ë§ì´ ë²Œì–´ì¡ŒìŠµ?ˆë‹¤.",
      reason: "?ìŠ¹?€ ê°•í•˜ì§€ë§?ì§€ê¸ˆì? ?Œë¦¼ ?ëŠ” ?¬ëŒ???•ì¸????? ë¦¬?©ë‹ˆ??",
    };
  }
  if (strongVolume && inEntryZone && trendUp) {
    return {
      action: "ì§„ì… ê°€??,
      tone: "buy",
      headline: "ê±°ë˜?‰ê³¼ ë¶„ë´‰ ì¶”ì„¸ê°€ ì§„ì… êµ¬ê°„?ì„œ ê°™ì´ ë¶™ì—ˆ?µë‹ˆ??",
      reason: `RVOL ${rvol !== null ? rvol.toFixed(1) + "ë°? : "-"} / ?„ì¼?€ë¹?${previousDayRatio !== null ? previousDayRatio.toFixed(1) + "ë°? : "-"} / 3ë´?ë³€??${pct(shortReturn)}`,
    };
  }
  if (strongVolume && breakout && trendUp) {
    return {
      action: "?ŒíŒŒ ?•ì¸",
      tone: "buy",
      headline: "ì§ì „ ë¶„ë´‰ ê³ ì  ?ŒíŒŒ?€ ê±°ë˜??ì¡°ê±´??ê°™ì´ ?¡í˜”?µë‹ˆ??",
      reason: `?„ì¬ê°€ê°€ ìµœê·¼ ?¨ê¸° ê³ ì ê¶Œì„ ?˜ì–´?°ê³  ê±°ë˜??ë°°ìˆ˜ê°€ ê°•í•©?ˆë‹¤. ?ì ˆ?€ ATR/VWAP ê¸°ì??¼ë¡œ ì§§ê²Œ ë´ì•¼ ?©ë‹ˆ??`,
    };
  }
  if (strongVolume && trendUp) {
    return {
      action: "ê´€??? ì?",
      tone: "watch",
      headline: "ê±°ë˜?‰ì? ê°•í•˜ì§€ë§?ì§„ì… ê¸°ì????¬í™•?¸ì´ ?„ìš”?©ë‹ˆ??",
      reason: "ê°€ê²©ì´ ì§„ì… êµ¬ê°„???¿ê±°???¨ê¸° ê³ ì ???¤ì‹œ ?˜ëŠ”ì§€ ?•ì¸?˜ì„¸??",
    };
  }
  return {
    action: "ê¸°ë‹¤ë¦¬ì„¸??,
    tone: "hold",
    headline: "ê±°ë˜???ëŠ” ?¨ê¸° ì¶”ì„¸ ?•ì¸???„ì§ ë¶€ì¡±í•©?ˆë‹¤.",
    reason: "RVOL, ?„ì¼ ?€ë¹?ê±°ë˜?? VWAP, ?¨ê¸° ê³ ì  ?ŒíŒŒê°€ ?™ì‹œ??ë¶™ì–´??ë¹ ë¥¸ ì§„ì… ? í˜¸ë¡?ë°”ë€ë‹ˆ??",
  };
}
...
function updateFastSignalCard(panel, decision) {
  ...
  const card = Array.from(panel.querySelectorAll("section, article, div"))
    .filter((node) => /?¨í? ê¸°ì? ? í˜¸/.test(node.textContent || ""))
...
function syncLegacyPriceMetrics(panel, levels) {
  setMetricByLabel(panel, "?„ì¬ê°€", krwMoneyFromUsd(levels.price));
  setMetricByLabel(panel, "ì§„ì… ê²€??êµ¬ê°„", `${krwMoneyFromUsd(levels.entryLow)} ~ ${krwMoneyFromUsd(levels.entryHigh)}`);
  setMetricByLabel(panel, "?ì ˆ ê¸°ì?", krwMoneyFromUsd(levels.atrStop));
  setMetricByLabel(panel, "1ì°??ŒíŒŒ ?•ì¸ê°€", krwMoneyFromUsd(levels.entryHigh));
  setMetricByLabel(panel, "2ì°??ŒíŒŒ ?•ì¸ê°€", krwMoneyFromUsd(levels.recentHigh * 1.035));
  setMetricByLabel(panel, "ëª©í‘œê°€", krwMoneyFromUsd(levels.entryHigh));
}
...
function candleChartSvg(bars, currentPrice, levels) {
  ...
  return `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="?ì²´ ìº”ë“¤ì°¨íŠ¸">
    ...
    ${candles}
    <text class="label" x="${width - pad - 120}" y="${pad + 12}">?„ì¬ ${money(last)}</text>
  </svg>`;
}
  ...
  try {
    ...
    chart.innerHTML = `
      <div class="kbk-pro-chart-head">
        <div><strong>${textEscape(symbol)} ?ì²´ ìº”ë“¤ì°¨íŠ¸</strong><span>ìµœê·¼ ë¶„ë´‰ ê¸°ì?, VWAP ?Œë? ?ì„  / ì§„ì…Â·?ì ˆ ê¸°ì????œì‹œ</span></div>
        <span>${new Date().toLocaleTimeString("ko-KR")}</span>
      </div>
      ${candleChartSvg(bars, price, { VWAP: vwap, ì§„ì…: entryLow, ?ì ˆ: atrStop })}
    `;
    const basis = document.createElement("section");
    basis.className = "kbk-pro-basis";
    basis.innerHTML = `
      <div><span>ì§„ì…ê°€ ê³„ì‚° ê·¼ê±°</span><b>${money(entryLow)} ~ ${money(entryHigh)}</b><small>VWAP ??? ì? + ìµœê·¼ 20ê°?ë¶„ë´‰ ê³ ì  ?ŒíŒŒê¶Œì„ ê¸°ì??¼ë¡œ ê³„ì‚°?ˆìŠµ?ˆë‹¤.</small></div>
      <div><span>ATR ê¸°ë°˜ ?ì ˆ</span><b>${money(atrStop)} ${stopPct !== null ? `(${pct(stopPct)})` : ""}</b><small>ìµœê·¼ 14ê°?ë¶„ë´‰ ATR ${atr ? money(atr) : "-"}?€ ì§ì „ ì§€ì§€? ì„ ?¨ê»˜ ë°˜ì˜?ˆìŠµ?ˆë‹¤.</small></div>
      <div><span>ê²€ì¦??¬ì¸??/span><b>VWAP / ê±°ë˜??/ ê³ ì  ?ŒíŒŒ</b><small>?„ì¬ê°€ê°€ VWAP ?„ë˜ë¡??´ë ¤ê°€ê±°ë‚˜ ?ŒíŒŒ ê±°ë˜?‰ì´ ?½í•˜ë©?? ê·œ ì§„ì…??ë³´ë¥˜?©ë‹ˆ??</small></div>
    `;
...
async function renderBacktestExpectation() {
  ...
  box.textContent = "?¹ë¥ /ê¸°ë?ê°??µê³„ë¥?ê³„ì‚°?˜ëŠ” ì¤‘ì…?ˆë‹¤.";
  target.prepend(box);
  try {
    ...
    box.innerHTML = `
      <strong>?„ì¬ ?„ë³´êµ?ê¸°ë?ê°??”ì•½</strong><br>
      ?œë³¸ ${sample.length}ê°?Â· 75???´ìƒ ${high.length}ê°?Â· ?‰ê·  ?ìˆ˜ ${Math.round(avgScore)}??Â· ?‰ê·  ?ìŠ¹ë¥?${pct(avgMove)} Â· ?‰ê·  ?„í—˜ ${Math.round(risk)}??br>
      ?¤ì œ ?¹ë¥ ?€ ?„ë˜ ?œê·¸??ê¸°ë¡???“ì¼?˜ë¡ ë°±í…Œ?¤íŠ¸ ?Œì´ë¸”ì—??ê²€ì¦ë©?ˆë‹¤. ì§€ê¸??œì‹œ???„ì¬ ?„ë³´êµ°ì˜ ì¡°ê±´ ê°•ë„ ?”ì•½?…ë‹ˆ??
    `;
  } catch (error) {
    box.textContent = `?¹ë¥ /ê¸°ë?ê°??µê³„ ê³„ì‚° ?¤íŒ¨: ${error.message}`;
  }
}
